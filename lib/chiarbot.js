...
const {info : log, verbose} = require("winston");
...
const CONNECT_BOT = auth.RETRIEVE_TOKENS.then(async () => {
    verbose(`Trying connection to IRC`);
    const OPTIONS = {
        identity: {
...
function onChatHandler(target, user, msg, self) {
    ...
    if (command) {
        ...
        if (filter) {
            verbose('channel command denied: ' + filter.failText);
        } else {
            ...
            if (filter) {
                if (filter.failText == 'dev-only command') {
                    ...
                    verbose('command denied: ' + filter.failText);
            } else {
